  ...
  return (
    <div className="space-y-8">
      ...
      <ResponsiveContainer width="100%" height={480}>
        <LineChart data={data} margin={{ top: 10, right: 70, bottom: 40, left: 10 }}>
          ...
          <XAxis
            ...
            label={{ value: "Equipment Quality", position: "insideBottom", offset: -15, style: { fontSize: 14, fontWeight: 500 } }}
          />
          <YAxis
            domain={yDomain}
            tickFormatter={(v: number) => v >= 1000 ? `${(v / 1000).toFixed(0)}k` : v >= 1 ? v.toFixed(0) : v.toFixed(2)}
            label={{ value: "Relative Probability", angle: -90, position: "center", dx: -25, style: { fontSize: 14, fontWeight: 500 } }}
          />
          <Tooltip
            formatter={(v: number, k) => [(v as number).toFixed(2), META[k as DemographicKey]?.label ?? k]}
