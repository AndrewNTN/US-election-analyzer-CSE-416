...
export function EIRejectedBallotsView({
  ...
  const { data, isLoading, isError } =
    useEIRejectedBallotsDataQuery(stateFipsPrefix);
  ...
  const chartData = useMemo(() => {
    ...
    return firstPoints.map((p, i) => {
      const row: Record<string, number> = { ratePct: p.x * 100 }; // Convert to percentage
      ...
      return row as { ratePct: number } & Partial<Record<string, number>>;
    });
  }, [data]);
